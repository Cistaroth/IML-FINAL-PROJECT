...
class KNN_movie_recomender:
    def __init__(
        self,
        data_location: str = "processed_data/userdata_embeddings.csv",
        max_features_kept: int = 20,
    ) -> None:
        """
        ...
        self.raw_data: pd.DataFrame = pd.read_csv(
            data_location
        )  # Could lead to data duplicated in memory, definitely something to keep in mind

        self.processed_data: pd.DataFrame = deepcopy(self.raw_data)
        self.count_vectorizer: CountVectorizer = CountVectorizer(
            max_features=max_features_kept, stop_words=None
        )

        self.y_dict: Dict[str, Optional[Any]] = {"y_train_full": None, "y_test": None}
        self.X_dict: Dict[str, Optional[pd.DataFrame]] = {
            "X_train_full": None,
            "X_test": None,
        }

        self.hp_dict: Dict[str, Any] = {
            "best_k": None,
            "k_values": None,
    ...
    def split_data(self) -> None:
        """
        This method will split the data into observations (y) and predictions (X) for both test and training data sets.

        :return: (None)
        """
        train_df, test_df = train_test_split(
            self.processed_data, test_size=0.2, random_state=42
        )

        # here, we find the median value of our rating, this will be the threshold that we will utilize to determine
        # if a movie is a "should watch" or a "shouldn't watch"
        ...
        self.y_dict["y_train_full"] = (train_df["rating"] > dynamic_threshold).astype(
            int
        )
        ...
        self.X_dict["X_train_full"] = train_df.drop(
            columns=cols_to_drop, errors="ignore"
        ).select_dtypes(include=["number"])
        self.X_dict["X_test"] = test_df.drop(
            columns=cols_to_drop, errors="ignore"
        ).select_dtypes(include=["number"])

    def _create_curated_string(self, obj: str, n_kept: Optional[int] = 3) -> List[str]:
        """
        Helper function to curate a list from ["Ricardo Rubert",...] to ["ricardorubert", ...].

        :param obj: (str) This is the string representation of the list that we are going to be fixing.
        :param n_kept: (Optional[int]) This is the number of values that we are going to keep from the top, so 3 means
                       we only keep 3 of the names. Defaults to 3.
        :return: (List[str]) The list of curated strings.
        """
    ...
    def ohe_lists(
        self, columns: List[str] = ["credits", "genres", "production_countries"]
    ) -> None:
        """
        One hot encode lists properly, while trying to avoid the curse of dimensionality.
        ...
        self.processed_data = pd.concat(
            [self.processed_data] + vectorized_dataframes, axis=1
        )

    def ohe_categorical_nonlist(
        self, columns_to_ohe: List[str] = ["language", "media_type"]
    ) -> None:
        """
        One hot encode categorical values that weren't in lists.
        ...
        self.processed_data = pd.get_dummies(
            self.processed_data, columns=columns_to_ohe, dtype=int
        )

    def hyperparameter_search(self, max_val_k: int = 50) -> None:
        """
        Method that searches all wanted values of K until the optimal one is found using the balanced accuracy metric.

        :param max_val_k: (int) This is the maximum value of K that we are going to search in. Defaults to 50.
        :return: (None)
        """
        ...
        for k in self.hp_dict["k_values"]:
            ...
            scores = cross_val_score(
                pipeline,
                self.X_dict["X_train_full"],
                self.y_dict["y_train_full"],
                cv=cv,
        ...
        self.hp_dict["best_score"] = self.hp_dict["mean_balanced_accuracy_scores"][
            best_score_index
        ]

        # We get our best K value and score value
        print(
            f"The Optimal K is: {self.hp_dict['best_k']} with a Validation Accuracy of {self.hp_dict['best_score']:.2%}"
        )

    def create_final_pipeline(self) -> None:
        """
        Creates the final pipeline utilizing the best K value found during hyperparameter search,
        fits it on the full training set, and calculates the final score on the test set.
        ...
        self.final_pipeline.fit(
            self.X_dict["X_train_full"], self.y_dict["y_train_full"]
        )

        # And obtain our final score
        self.final_score = self.final_pipeline.score(
            self.X_dict["X_test"], self.y_dict["y_test"]
        )
        print(f"Final Test Accuracy: {self.final_score:.2f}")

    def calculate_final_prediction(self) -> None:
        """
        Calculates the predictions (y_pred) and probabilities (y_prob) for the test set
        using the trained final pipeline.
    ...
    def output_non_dim_columns_with_example_data(self) -> None:
        """
        Helper function to just see the columns that aren't embeddings and an example value of them.
        Mainly used for debugging.

        :return: (None)
        """
        [
            print("Name:", column, "Val:", self.processed_data[column][0])
            for column in self.processed_data
            if "dim" not in str(column)
        ]
